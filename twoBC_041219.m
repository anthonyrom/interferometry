...
while i <= n
    ...
    R_1(i) = str2double(fscanf(lockin2));
    i = i + 1
    pause(0.5) % takes data every 0.5 s
end

csvwrite('1_PPol_041219.csv', R_1)
R_1_avg = mean(R_1)
R_1_std = std(R_1)
...
while i <= n
    fprintf(lockin2, 'OUTP?3 /n');
    R_1_both_1(i) = str2double(fscanf(lockin2));
    fprintf(lockin1, 'OUTP?3 /n');
    R_2_both_1(i) = str2double(fscanf(lockin1));
    i = i + 1
    pause(dt)
end

csvwrite('Both_1_PPol_041219.csv', [R_1_both_1, R_2_both_1])
...
while i <= n
    fprintf(lockin1, 'OUTP?3 /n');
    R_2(i) = str2double(fscanf(lockin1));
    i = i + 1
    pause(dt) % takes data every 0.5 s
end

csvwrite('2_PPol_041219.csv', R_2)
R_2_avg = mean(R_2)
R_2_std = std(R_2)
...
while i <= n
    fprintf(lockin2, 'OUTP?3 /n');
    R_1_both_2(i) = str2double(fscanf(lockin2));
    fprintf(lockin1, 'OUTP?3 /n');
    R_2_both_2(i) = str2double(fscanf(lockin1));
    i = i + 1
    pause(dt)
end

csvwrite('Both_2_PPol_041219.csv', [R_1_both_2, R_2_both_2])
